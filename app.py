...
def home():  # Ruta principal
    return redirect(url_for('Usuario'))  # Muestra la plantilla de inicio
...
def guardar_pedido_en_db(lista_productos, num_cel, solicitud ,metodo_pago, metodo_entrega):
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id_pedido
        ...
        id_pedido = cursor.fetchone()[0]  # ← 🔹 ESTE ES EL ID REAL DEL PEDIDO INSERTADO
...
def alta():
    if request.method == "POST":
        ...
        DireccionID = request.form.get("direccion_existente") 
        ...
        try:
            cursor.execute("""
                INSERT INTO tbl_cliente (id_cel, nombre, apellido, apodo, tipo, dirreccion_c)
                VALUES (%s, %s, %s, "", 'a', %s)
            """, (Numero, Nombre, Apellido, DireccionID))
            ...
            cursor.close()
            db.close()
    return render_template("admin.html", modo="alta")
    ...
    if request.method == "POST":
        ...
        try:
            cursor.execute("""
                INSERT INTO tbl_cliente (id_cel, nombre, apellido, apodo, tipo, dirreccion_c)
                VALUES (%s, %s, %s, %s, 'u', %s)
            """, (Numero, Nombre, Apellido, Apodo, DireccionID))

            db.commit()
            flash("Cliente vinculado a la dirección correctamente", "success")
        except Exception as e:
            db.rollback()
            flash(f"Error al crear cliente: {e}", "danger")
        finally:
...
def PedidoSuc():
    ...
    if request.method == "POST":
        ...
        for id_producto in productos:
            ...
            precio = producto.get('precio') or producto.get('Precio', 0)
